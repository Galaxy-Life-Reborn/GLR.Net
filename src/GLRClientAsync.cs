...
namespace GLR.Net
{
    public partial class GLRClient
    {
        ...
        public async Task<Profile> GetProfileAsync(ulong id)
        {
            ...
            profile.Url = $"https://launcher.galaxylifereborn.com/profile/{profile.Username.Replace(" ", "%20")}";
            profile.ImageUrl = $"https://launcher.galaxylifereborn.com/uploads/avatars/{profile.Id}.png?t={currentUnixTime}";
        ...
        {
            var result = await _webClient.GetAsync($"https://launcher.galaxylifereborn.com/api/userinfo?u={id}&t=c");
